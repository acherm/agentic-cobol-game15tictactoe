       ...
       PROGRAM-ID. GAME15.
      *
      * Computes all possible games in the Game of 15.
      * Two players alternate picking numbers 1-9 (no repeats).
      * A player wins when three of their numbers sum to 15.
      ...
      *
      * All 8 triples of {1..9} that sum to 15:
      ...
      * Each row maps input number N to its image under that
      * symmetry: identity, 90CW, 180, 270CW, flipH, flipV,
      ...
      *
           DISPLAY "Game of 15 - Possible Games"
           DISPLAY "==========================="
      ...
      *
       DFS-STEP.
           IF NEXT-TRY(DEPTH) > 9
      *        All numbers tried at this depth - backtrack
               IF DEPTH = 1
                   MOVE 1 TO DONE-FLAG
               ELSE
      ...
      *            Number already taken - skip
                   ADD 1 TO NEXT-TRY(DEPTH)
               ELSE
      *            Determine current player (odd depth=P1, even=P2)
                   DIVIDE DEPTH BY 2
      ...
      *                Undo and try next
                   ...
                   ELSE
                       IF DEPTH = 9
      *                    All numbers used, no winner - draw
                           ADD 1 TO DRAWS
                           IF UNIQUE-MODE = 1
                               MOVE 9 TO GAME-LEN
                               PERFORM CHECK-CANONICAL
      ...
      *
       CHECK-WIN.
           ...
           .
      *
      * Check if the current game (CHOSEN(1..GAME-LEN)) is the
      * lexicographically smallest among all 8 symmetric variants.
      * Sets IS-CANONICAL to 1 if yes, 0 if no.
       CHECK-CANONICAL.
